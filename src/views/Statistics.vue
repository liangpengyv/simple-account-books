...
<template>
  ...
  <n-tabs v-else type="line" animated display-directive="show">
    <n-tab-pane v-for="(data, index) in currentCategoryStatisticsData" :key="index" :name="index"
      :tab="billType[index]">
      ￥
      <n-number-animation :from="0.0" :to="data.total" :precision="2" :duration="300" />
      <ul v-if="data.group.length > 0">
        <li v-for="item in data.group" :key="item.categoryId">
          {{ categoryStore.categoriesDict[item.categoryId].name }}
          ({{ item.size }}笔):
          【{{ item.total }}】
          [{{ item.percent.toFixed(2) }}%]
          <n-progress type="line" :percentage="item.percent" :show-indicator="false" />
        </li>
      </ul>
      <n-empty v-else description="本月暂时没有交易" />
    </n-tab-pane>
  </n-tabs>
